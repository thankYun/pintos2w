...
/**
 * @param name 스레드 이름
 * @param priority 기본 우선순위
 * @param function 실행할 함수 이름
 * @param aux 매개변수(실행할 함수에 들어갈)
 * */
tid_t thread_create(const char *name, int priority, thread_func *function, void *aux)
{
	...
	/* Initialize thread. */
	init_thread(t, name, priority); // 4KB의 단일 공간에 스레드 구조체 초기화
	tid = t->tid = allocate_tid();	// 스레드의 고유 ID를 할당한다.

	/* Call the kernel_thread if it scheduled.
	 * Note) rdi is 1st argument, and rsi is 2nd argument. */
	t->tf.rip = (uintptr_t)kernel_thread;	//커널 스레드의 기준으로 사용되는 함수(스레드 시작 시 필요한 초기화 수행)
	t->tf.R.rdi = (uint64_t)function; // 실행하려는 함수의 주소
	t->tf.R.rsi = (uint64_t)aux;	  // fn_copy를 인자로 넣음
	t->tf.ds = SEL_KDSEG;
	t->tf.es = SEL_KDSEG;
	t->tf.ss = SEL_KDSEG;
	t->tf.cs = SEL_KCSEG;
